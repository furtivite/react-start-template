...
export const getBeautifulNumber = (value?: unknown, separator: ' ' = ' ') =>
...
type TSomeArrItem = { value: unknown; number: unknown };

export const getNumberedArray = (arr: TSomeArrItem[]) => arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: TSomeArrItem[]) => arr.map(({ value, number }) => `${value}_${number}`);
...
export const transformCustomers = (customers: unknown[]) => {
  return customers.reduce((acc: ICustomer[], customer: ICustomer) => {
